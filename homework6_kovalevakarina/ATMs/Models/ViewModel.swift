...
class ViewModel: NSObject {
        ...
        if NetworkMonitor.shared.isConnected {
            ...
            service.getAtmsList { success, model, _ in
                if success, let atms = model {
                    self.atms = atms
                    self.processFetchedAtm(atms: atms)
                } else {
                    errors.append(0)
                }
                self.group.leave()
    ...
    private func processFetchedAtm(atms: [ATM]) {
        for atm in atms {
            AtmCoreData.createOrUpdate(item: atm, with: AppDelegate.sharedAppDelegate.coreDataStack)
        }
